...
if __name__ == "__main__":
  ...
  flags.DEFINE_integer("num_epochs", 500, '''5,'''
...
def get_input_data_tensors(reader,
  ...
  with tf.name_scope("train_input"):
    ...
    training_data_flip = [
        reader.prepare_reader_flip(filename_queue) for _ in range(num_readers)
    ]
    training_data_rotR = [
        reader.prepare_reader_rotR(filename_queue) for _ in range(num_readers)
    ]
    training_data_rotL = [
        reader.prepare_reader_rotL(filename_queue) for _ in range(num_readers)
    ]

    # ignore thread 8
    training_data += training_data_flip + training_data_rotR + training_data_rotL

    # retain thread 8
    #for i in range(num_readers):
    #    training_data[i] += training_data_flip[i]
    #    training_data[i] += training_data_rotR[i]
    #    training_data[i] += training_data_rotL[i]
...
def build_graph(reader,
  ...
  # decline learning rate on decay
      ...
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus!=1 else "/gpu:0")):
          ...
          if "loss" in result.keys():
            ...
            #tmp_tower_labels = reader.prepare_reader.dense_to_one_hot(tower_labels[i], 2)
            #tmp_tower_labels = tf.map_fn(lambda x: tf.cast(slim.one_hot_encoding(x, 2), tf.int32), tower_labels[i])
            #tmp_tower_labels = tf.reshape(tmp_tower_labels, [-1, 2])
          ...
          if "regularization_loss" in result.keys():
            ...
            #weight_reg = tf.trainable_variables()
            #l2_regularizer = slim.l2_regularizer(scale=0.0005, scope=None)
            #reg_loss = slim.apply_regularization(l2_regularizer, weight_reg)
...
#                           task_as_string(self.task))
  ...
  def build_model(self, model, reader):
    ...
    build_graph(reader=reader,
                ...
                train_data_pattern="/ssd2/google_challenge/catdog/train/*",
